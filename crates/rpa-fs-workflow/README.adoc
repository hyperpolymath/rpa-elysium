...
Filesystem workflow automation CLI for link:../../README.adoc[RPA
Elysium].

=== Overview

`+rpa-fs+` watches directories for file system events (create, modify,
delete, rename) and executes configured actions — copy, move, archive,
delete, rename, or plugin-based operations.
...
Workflow configs are JSON or https://nickel-lang.org/[Nickel] files.
...
See link:../../examples/workflow.ncl[`+examples/workflow.ncl+`] for a
...
|===
|Action |Description
|`+copy+` |Copy file to destination (with optional overwrite and
...
|`+rename+` |Rename file using `+{name}+` and `+{ext}+` pattern
substitution

|`+plugin+` |Execute a WASM plugin action (requires rpa-plugin
integration)
|===
