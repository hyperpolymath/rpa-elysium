...
* *Existing ABI proofs*: `+src/abi/*.idr+` including domain-specific
`+ProvenFSM.idr+` and `+ProvenQueue.idr+`
...
* Finite state machine for RPA workflow execution — already has Idris2
definitions
* Audit: are all transitions proven valid? Are invalid transitions
statically rejected?
...
* Prove: error handling preserves system state (no partial effects on
failure)
...
* *Idris2* (already in use with ProvenFSM and ProvenQueue — complete the
proofs)

=== Priority

*HIGH* — RPA system performing automated file operations. Incorrect FSM
transitions or lost queue actions could cause data loss. The Idris2
proof infrastructure is already in place — completing it is the
highest-ROI work.
